 ...
 const fetchNowPlayingMovies = async() => {
        try{
            const {data} = await axios.get('/api/show/now-playing', {
              headres: {Authorization: `Bearer ${await getToken()}`}})
          if(data.success) {
      ...
      try{
        ...
        if(!selectedMovie || Object.keys(dateTimeSelection).length === 0
          ...
          const payload = {
           ...
           const {data} = await axios.post('/api/show/add', payload, {headres: {
  ...
  return nowPlayingMovies.length > 0 ? (
    ...
    text-white px-8 py-2 mt-6 rounded hover:bg-primary/90 transition-all 
    cursor-pointer">Add Show</button>
    </>
  ) : <Loading />
}
